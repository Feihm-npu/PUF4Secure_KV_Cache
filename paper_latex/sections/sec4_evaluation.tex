...
Real-context seeded prompts produce the same injection result, on both public-news and real-email corpora. Table~\ref{tab:real-context-pii} shows that both Qwen3-0.6B and Llama-3.2-1B leak exact keywords in 69 of 300 plaintext CC-News contexts (23.0\%), whereas protected native caches leak zero exact keywords. Exact digit leakage drops from 50 of 300 to 0 of 300 on Qwen3 and from 45 of 300 to 0 of 300 on Llama. To move beyond public news to genuine private-style correspondence, we repeat the benchmark with contexts drawn from the Enron email corpus: plaintext caches leak exact keywords in 14 of 200 prompts (7.0\%) and exact digit strings in 12 of 200, while protected native caches again leak zero. The plaintext leak rate is lower on Enron because real email text is a noisier injection setting than clean news prose, but the protection result is unchanged at exact zero. Finally, to drop the synthetic-secret assumption entirely, we mine the email addresses and phone numbers that \emph{genuinely occur} in the Enron emails (regex over the cached text) and score recovery of those real entities rather than a seeded one: plaintext caches leak 35 of 200 real entities on Qwen3-0.6B (17.5\%; $22\%$ of emails, $7\%$ of phones) and 25 of 200 on Llama-3.2-1B (12.5\%), while protected native caches leak zero real entities on both models. Real, naturally occurring PII is therefore recovered from plaintext caches and eliminated by protection, not just synthetic secrets we planted.

\begin{table*}[t]
\centering
\caption{Real-context PII benchmark. Protected native caches eliminate exact keyword and digit leakage for synthetic secrets embedded in cached CC-News articles and Enron emails, and---most realistically---for real email addresses and phone numbers mined from the Enron emails themselves.}
...
\midrule
Qwen3-0.6B & Plain (Enron, real entities) & 200 & 35 (17.5\%) & 4 (2.0\%) \\
Qwen3-0.6B & Protected (Enron, real entities) & 200 & \best{0 (0.0\%)} & \best{0 (0.0\%)} \\
Llama-3.2-1B & Plain (Enron, real entities) & 200 & 25 (12.5\%) & 7 (3.5\%) \\
Llama-3.2-1B & Protected (Enron, real entities) & 200 & \best{0 (0.0\%)} & \best{0 (0.0\%)} \\
\bottomrule
\end{tabular}
\end{table*}
...
Llama-2-7B & +0.0000010 & 0.0000 & 3/3 \\
...
\textbf{Answer to RQ3:} \sys{} preserves behavior in fp32 across Qwen3-0.6B, Llama-3.2-1B, Llama-2-7B, Qwen2.5-1.5B, and Qwen2.5-7B on perplexity, HellaSwag, MMLU, and long greedy decoding, and the affine-mask path is fp32-exact on the same tasks; low precision requires care because 56.25\% of Qwen3-0.6B 32-token bf16 continuations diverge, though fp16 cuts this to 15.6\% (Section~\ref{sec:fused-kernel} shows the fused affine path diverges only $14.1\%$ at $64$ tokens) and is the relevant precision for a FlashAttention deployment.
...
\subsection{RQ12: Fused de-masking kernel}
\label{sec:fused-kernel}

The affine mask is the harder serving case because it must be subtracted between the cache read and the QK/AV products. An \emph{eager} de-mask---a separate full-cache elementwise pass per decode step---costs a second traversal of the KV cache and accounts for the $-33\%$ overhead of the affine row in Table~\ref{tab:baselines}. We close that gap with a fused Triton flash-decoding kernel that regenerates the PUF mask in-register via the same counter-based Philox RNG as the write path ($M[\text{pos}\cdot H D+h\cdot D+j]=\sigma(\mathrm{randn}(s_a,c)+\mathrm{randn}(s_b,c))/\sqrt 2$, keyed by a 126-bit PUF-derived seed pair) and subtracts it in the SRAM epilogue of the KV load, so de-masking piggybacks on the load attention already performs: no mask is stored in HBM and no separate de-mask pass is launched.

\textbf{Split-K is necessary for long context.} A naive single-program-per-head kernel ($\text{grid}{=}(B,H_q)$) places only 16 programs on the 108-SM A100 for Qwen3-0.6B ($H_q{=}16$); each program then serially loops the full sequence, so occupancy collapses and overhead worsens from $-33\%$ at prefill 256 to $-69\%$ at prefill 1024. The split-K (flash-decoding) variant $\text{grid}{=}(B,H_q,n_{\text{splits}})$ partitions the sequence dimension across $n_{\text{splits}}\le 8$ programs per head, each producing a partial $(m_i,\ell_i,\text{acc}_i)$ over its chunk; a second reduction kernel merges the $n_{\text{splits}}$ partials per $(B,H_q)$ via the log-sum-exp combine. The mask counters are absolute position-indexed, so each split regenerates exactly the mask rows the write path produced---the split-K output matches the single-program kernel to within $10^{-6}$.

\begin{table}[t]
\centering
\caption{RQ12. Fused de-masking decode overhead on Qwen3-0.6B (fp32, 128-token decode, 8 repeats, A100-80GB). The eager de-mask is a separate per-step pass; the fused kernel regenerates+subtracts the mask in SRAM. Split-K eliminates the naive kernel's long-context collapse.}
\label{tab:fused-kernel}
\begin{tabular}{@{}lcccc@{}}
\toprule
& \multicolumn{2}{c}{prefill 256} & \multicolumn{2}{c}{prefill 1024} \\
\cmidrule(lr){2-3}\cmidrule(lr){4-5}
Kernel path & tok/s & $\Delta$ & tok/s & $\Delta$ \\
\midrule
plain fp32                 & 33.9 & ---       & 34.1 & ---       \\
orthogonal (eager)         & 28.7 & $-15.4\%$ & 28.8 & $-15.5\%$ \\
affine, eager de-mask      & 22.5 & $-33.7\%$ & 22.7 & $-33.3\%$ \\
affine, fused (split-K)    & \best{24.2} & \best{$-28.6\%$} & \best{23.8} & \best{$-30.2\%$} \\
\bottomrule
\end{tabular}
\end{table}

\textbf{The fused kernel is numerically identical and consistently faster than eager.} Across 64-prompt/64-token greedy-decode suites the fused and eager affine paths diverge from plain at identical rates (Qwen3 $0.0/0.0$, Llama $0.0/0.0$) and produce token-identical output on both models, confirming the in-kernel Philox mask matches the write path bit-for-bit. Table~\ref{tab:fused-kernel} shows the fused kernel is $7$--$5\%$ faster than eager at both prefill lengths and, critically, removes the long-context regression: the naive single-program kernel degraded from $-33\%$ to $-69\%$ going from prefill 256 to 1024, while the split-K kernel stays flat at $\approx{-}29\%$. The residual gap to the orthogonal path ($-15\%$) is the inherent cost of the fp32 Philox RNG and mask subtraction that the de-mask epilogue adds to every cache load---the price of removing the basis-invariant class without a separate HBM pass. Under fp16 model weights (fp32 cache retained, identical 64-prompt/64-token conditions), the fused affine path diverges from plain on $14.1\%$ of continuations---lower than both the eager affine path and the orthogonal path, which diverge at the \emph{same} $20.3\%$ rate. The mask subtraction is performed in fp32 inside the wrapper and therefore adds no precision loss beyond the rotation itself; the fused kernel's lower divergence (different floating-point accumulation order, Triton online softmax versus PyTorch batch softmax) produces logits that flip fewer greedy tokens versus the plain fp16 reference.

\textbf{Answer to RQ12:} The affine de-mask fuses into a Triton flash-decoding kernel that is numerically indistinguishable from eager ($10^{-6}$ match, token-identical on Qwen3-0.6B and Llama-3.2-1B) and eliminates the naive kernel's long-context collapse via split-K parallelism; the resulting $\approx 29\%$ decode overhead is $4$--$5$ percentage points lower than eager and is stable across prefill lengths, leaving a $\approx 14$pp residual versus orthogonal that reflects the inherent fp32 de-mask ALU cost. Paged and flash-attention integration remain future work.

\subsection{Open Evaluation Gaps}

The Route~B experiments close the largest earlier gaps: the candidate audit now extends to $N=1000$ (RQ7) and confirms the advantage is flat in the candidate-set size, the differencing attack and its nonce fix are validated on real caches (RQ8), the head-to-head against encrypt-at-rest quantifies the materialization/throughput tradeoff (RQ9), the PUF/FE assumptions are discharged in simulation (RQ10), and the fused affine de-masking kernel is implemented, validated, and benchmarked (RQ12). Two gaps remain. First, the PUF is simulated; RQ10 discharges the assumptions in software, but real FPGA traces are still needed to validate physical min-entropy, reliability, and helper-data leakage. Second, the fused kernel targets contiguous-cache decode (the $O(n)$-per-step regime where the de-mask overhead lives); integration with paged and flash-attention serving kernels remains engineering work.
